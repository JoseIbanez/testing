...
async def async_main():
    ...
    async for event in events_async:
        ...
        if is_pending_auth_event(event):
            logger.info("Authentication required by agent, event: %s", event.content)
        ...
        parts=[
            types.Part(
                function_response=types.FunctionResponse(
                    ...
                    # The payload containing all necessary authentication details for generating the authentication token.
                    response=auth_config.model_dump(),
                )
            )
        ],
    )
    logger.info("auth_content: %s", auth_content)
    logger.info("auth_content.response: %s", auth_config.model_dump(mode='json'))
