...
echo "WEBUI_USER='-e WEBUI_ADMIN_USER=${open_webui_user}'" >> /etc/open-webui.d/openwebui.env
echo "WEBUI_PASS='-e WEBUI_ADMIN_PASSWORD=${open_webui_password}'" >> /etc/open-webui.d/openwebui.env
...
ExecStart=/usr/bin/docker run -p 80:8080 $WEBUI_USER $WEBUI_PASS $GPU_FLAG $OPENAI_BASE $OPENAI_KEY -e RAG_EMBEDDING_MODEL_AUTO_UPDATE=true -v /etc/open-webui.d:/root/.open_web_ui -v /etc/open-webui.d:/app/backend/data --name %n ghcr.io/open-webui/open-webui:ollama
